...
{
    public class ThroughputReqRepMessage : TestBase
    {
        public const int MsgSize = 64 * 1024;

        public const long NumberOfMessages = 50_000;

        //private const string PipeName = "inproc://" + nameof(ThroughputReqRepMessage);
        private const string PipeName = "ipc:///tmp/" + nameof(ThroughputReqRepMessage);
        ...
        private void Reply()
        {
            using (var rep = new ReplySocket())
            {
                ...
                for (var i = 0; i < NumberOfMessages; i++)
                {
                    ...
                    NNG.SendMessage(rep.Socket, received);
                }
        ...
        private unsafe void Request(object obj)
        {
            ...
            using (var req = new RequestSocket())
            {
                ...
                for (var i = 0; i < NumberOfMessages; i++)
                {
                    ...
                    var received = NNG.ReceiveMessage(req.Socket);
